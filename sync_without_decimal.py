import random
PRO_SPEED = 200000000.0
MASTER_ERR = 0.000005 # error of master node
SLAVE_ERR  = 0.000005 # error of slave node
PRO_ERR = 0.01 # error of propagation process 
SYNC_CYCLE = 1.0 # sync cycle, master fre as well 0.2 above
SLAVE_RANGE = 0.000005 # slave fre range
SLAVE_FRE = 1.0 + random.uniform(-1.0 * SLAVE_RANGE,1.0 * SLAVE_RANGE)
TOTAL_CYCLE = 30 # to see how long the sync process undergo
...
def slave(slaveClock,masterClock):
	...
	slaveClock[1] = (slaveClock[1]) + (masterClock[2]) * ((SLAVE_FRE) / (SYNC_CYCLE)) * ( 1 + err )
	return slaveClock

def sync(masterClock,slaveClock):
	global PRO_ERR, SYNC_CYCLE, SLAVE_FRE, SLAVE_ERR, MASTER_ERR, PRO_SPEED
	...
	delta_t = 0.000002 # time in slave node before the sync process
	...
	#print "master1",masterClock[1]
	temp = (real_propagation_time) + (delta_t) * (1 + (random.uniform(-1*MASTER_ERR,1*MASTER_ERR)))
	#print "temp",temp,real_propagation_time,delta_t,MASTER_ERR
	masterClock[1] = (masterClock[1]) + temp
	#print "master1",masterClock[1]
